...
#include "jtwain.h"    // JTWAIN header with JNI function prototypes.
...
static jobject xferDIB8toImage  (LPBITMAPINFOHEADER lpbmih, JNIEnv *env);
               ...
               (JNIEnv *env, jclass clazz)
{
   ...
   if (rc == TWRC_FAILURE)
       throwJTE (env, "Unable to display user interface "
                 "(selectSourceAsDefault)");
...
static void throwJTE (JNIEnv *env, const char *msg)
{
   ...
   jclass clazz = env->FindClass ("net/javajeff/jtwain/JTwainException");
   ...
   if (clazz == 0)
   {
       MessageBox (0, msg, "JTWAIN", MB_OK);
       env->FatalError ("Could not find net.javajeff.jtwain.JTwainException");
   }
   else 
...
static jobject xferDIB8toImage (LPBITMAPINFOHEADER lpbmih, JNIEnv *env)
{
   ...
   if (pixelsArray == 0)
   {
       throwJTE (env, "Insufficient memory (xferDIB8toImage)");
       return (jobject) 0;
   }
   ...
   if (obj1 == 0)
   {
       throwJTE (env, "Can't create java.awt.image.MemoryImageSource object "
                 "(xferDIB8toImage)");
       return (jobject) 0;
   }
   ...
   if (clazz == 0)
   {
       throwJTE (env, "Can't find java.awt.Toolkit class");
       return (jobject) 0;
   }
